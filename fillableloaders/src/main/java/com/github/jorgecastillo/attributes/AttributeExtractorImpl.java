...
public class AttributeExtractorImpl implements AttributeExtractor {
    ...
    private AttributeExtractorImpl(WeakReference<Context> weakContext, WeakReference<AttributeSet> weakAttrs) {
    ...
    private TypedArray attributeArray() {
        if (weakAttributeArray == null) {
            weakAttributeArray = new WeakReference<>(
                context().getTheme().obtainStyledAttributes(weakAttrs.get(), R.styleable.FillableLoader, 0, 0));
        }
    ...
    @Override
    public int getPreFillColor() {
        return attributeArray().getColor(R.styleable.FillableLoader_fl_preFillColor,
            context().getResources().getColor(R.color.preFillColor));
    }
