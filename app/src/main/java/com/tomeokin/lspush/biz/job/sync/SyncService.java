...
public class SyncService extends Service {
    ...
    private static final long SHOULD_REFRESH = EXPIRE_TIME / 3;
    ...
    public static boolean isServiceActive(Context context) {
        final Intent i = new Intent(context, SyncService.class);
    ...
    public SyncComponent component() {
        if (mComponent == null) {
            mComponent =
                DaggerSyncComponent.builder().appComponent(getAppComponent()).syncModule(new SyncModule()).build();
        }
        return mComponent;
    }
    ...
    public IBinder onBind(Intent intent) {
        ...
        Timber.i("onBind extra_job_id is %d", mJobId);
    ...
    public void sync(@Nullable final Callback callback) {
        ...
        Timber.i("getAccessResponseObservable start");
        mLocalUserInfoAction.getAccessResponseObservable()
            ...
            .subscribe();
        Timber.i("getAccessResponseObservable end");
    }
    ...
    private void refresh(final AccessResponse old, final Callback callback) {
        ...
        if (!diff.isNegative()) { // no need refresh
            ...
            if (now.isAfter(targetExpire) && shouldUpdateRefreshToken(now, old.getRefreshTime())) {
                // need refresh by password
                Timber.i("need refresh by password");
                updateRefreshToken(old, callback);
            } else {
                // need refresh by refresh token
                Timber.i("need refresh by refresh token");
                updateExpireToken(old, callback);
            }
        }
    }

    private boolean shouldUpdateRefreshToken(LocalDateTime now, long refreshTime) {
    ...
    private void updateRefreshToken(final AccessResponse old, final Callback callback) {
        mRefreshTokenAction.get().refreshRefreshToken(old)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .doOnError(new Action1<Throwable>() {
                @Override
    ...
    private void updateAccessResponse(final AccessResponse accessResponse, final Callback callback) {
        mLocalUserInfoAction.updateAccessResponseObservable(accessResponse)
            ...
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe(new Action1<AccessResponse>() {
                @Override
                public void call(AccessResponse accessResponse) {
                    Timber.i("subscribe");
                    callback.onSuccess();
